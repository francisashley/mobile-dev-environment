...
{constructor({reload=!0,hardReload=!0,logbox=!0,logErrors=!0}){if(reload===!0){this.reload=new MDEReload({hardReload})}
if(logbox===!0){this.logbox=new MDELogbox({logErrors})}}}
class MDEHelpers
{getDB(key){return localStorage[key]?JSON.parse(localStorage[key]):null}
setDB(key,val){return localStorage[key]=JSON.stringify(val)}
setupDB(keyVals){for(let key in keyVals){localStorage[key]=JSON.stringify(keyVals[key])}}
...
scrollInfo(elem){const{scrollTop,scrollHeight,clientHeight}=elem;return{top:scrollTop,bottom:scrollTop+clientHeight,height:clientHeight,atTop:scrollTop===0,atBottom:scrollHeight-scrollTop<=clientHeight,fullHeight:scrollHeight}}
returnTraceFromError(error){const stack=error.stack.split("\n")[2];const split=/\(([^)]+)\)/.exec(stack)[1].split(":");return{fileName:split[2].replace(/^.*[\\\/]/,''),filePath:split[0]+':'+split[1]+':'+split[2],lineNumber:split[3]}}}
...
{constructor(options){super();const{setupDB,getDB}=this;const{logErrors}=options;setupDB({logboxOpen:getDB('logboxOpen')||!1,logboxHeight:getDB('logboxHeight')||40});this.buildLogboxButton();this.buildLogbox();window.console.log=(message)=>{const trace=this.returnTraceFromError(new Error);this.log(message,trace)};if(logErrors===!0){window.onerror=(message,filePath,lineNumber)=>{const fileName=filePath.replace(/^.*[\\\/]/,'');this.log(message,{fileName,filePath,lineNumber,isError:!0})}}}
...
this.setHeight(height);const closeButton=fetch('close-logbox');const resizeButton=fetch('resize-logbox');window.addEventListener('resize',(e)=>{this.setHeight(this.height)},!1);closeButton.addEventListener('click',(e)=>{this.close()},!1);resizeButton.addEventListener('touchstart',(e)=>{resizeButton.classList.add('pressed');this.resize(e)},!1);resizeButton.addEventListener('touchend',(e)=>{resizeButton.classList.remove('pressed')},!1)}
get state(){return this.getDB('logboxOpen')}
get height(){return this.getDB('logboxHeight')}
get minHeight(){return this.fetch('resize-logbox').offsetHeight}
get maxHeight(){return window.innerHeight-this.fetch('reload').offsetHeight-20}
setHeight(height){const{setDB,fetch,minHeight,maxHeight,returnInRange}=this;height=returnInRange(height,minHeight,maxHeight);setDB('logboxHeight',height);fetch('logbox').style.height=height+'px'}
open(){const{fetch,setDB}=this;setDB('logboxOpen',!0);fetch('open-logbox').classList=!0;fetch('logbox').classList=!0}
close(){const{fetch,setDB}=this;setDB('logboxOpen',!1);fetch('open-logbox').classList=!1;fetch('logbox').classList=!1;console.log(fetch('open-logbox').classList)}
