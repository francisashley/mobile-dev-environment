module.exports = function logtray(options, DB) {
  ...
  self.icon = {
    toggleTray:
      '<svg fill="#000000" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M21 3H3c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H3v-3h18v3z"/></svg>'
  };
  self.elements = {
  ...
  self.elements.tray = crel(
    "div",
    { id: "mde-logtray", class: self.status },
  ...
  function displayLog({ message, filePath, fileName, lineNumber, type = "log" }) {
    ...
    const lastLogMessage =
      typeof lastLogElement === "object"
        ? lastLogElement.querySelector(".mde-log-message-full").innerHTML
        : null;
    const incrementLastMessage =
      self.elements.logs.children.length > 0 && lastLogMessage === message;
    ...
    if (incrementLastMessage) {
      ...
      const log = crel(
        ...
        crel(
          "a",
          { class: "mde-log-trace", href: filePath, target: "_blank" },
          fileName + ":" + lineNumber
        ),
